...
export class Migration20241118083758 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create table "user" ("id" varchar(255) not null, constraint "user_pkey" primary key ("id"));`,
    );

    this.addSql(
      `create table "cart" ("id" uuid not null default gen_random_uuid(), "is_deleted" boolean not null default false, "user_id" varchar(255) not null, constraint "cart_pkey" primary key ("id"));`,
    );

    this.addSql(
      `create table "order" ("id" uuid not null default gen_random_uuid(), "status" text check ("status" in ('created', 'completed')) not null default 'created', "total" int not null, "user_id" varchar(255) not null, "cart_id" uuid not null, constraint "order_pkey" primary key ("id"), constraint order_total_check check (total > 0));`,
    );
